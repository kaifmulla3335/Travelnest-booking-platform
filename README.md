...
<p align="center">
  ...
  <img alt="Groq AI" src="https://img.shields.io/badge/Groq_AI-LLaMA_3.3-F55036?logo=meta&logoColor=white&style=flat-square" />
...
A full-stack **travel booking platform** with a real-world, industry-style booking lifecycle — built with **Spring Boot 3 + Spring Security (JWT)** on the backend and **React 18 + Tailwind CSS** on the frontend, with **Razorpay** payment integration, automated PDF document generation, live QR-code ticket verification, an **AI-powered travel chatbot** (Groq LLaMA 3.3), and full CI/CD deployment.
...
- 🤖 **AI Travel Chatbot** (Groq LLaMA 3.3)
  - Floating chat widget available on all pages — no login required
  - Context-aware: AI is given the actual package list from the database (RAG-lite pattern)
  - Answers questions about packages, pricing, booking process, cancellation policy, and travel tips
  - Responds in English or Hindi based on user's language
  - Typing indicator, suggested quick questions, auto-scroll
- 🛠️ **Admin Dashboard**
...
| **AI** | Groq API (LLaMA 3.3 70B) via Spring RestClient — context-aware travel assistant |
...
│       │   ├── ChatController.java         → AI chatbot endpoint (/api/chat/message)
...
│       │   ├── ChatService.java            → Groq AI integration (RAG-lite: DB packages as context)
...
└── Frontend/
    ...
    └── src/
        ...
        └── components/
            ├── common/
            │   ├── ChatWidget.jsx          → AI chatbot floating widget (all pages)
...
| `GROQ_API_KEY` | Groq AI API key for the travel chatbot | must be set |
...
User types message in chat widget
         ↓
POST /api/chat/message (public endpoint — no login required)
         ↓
ChatService.java:
  1. Fetches all packages from DB (real-time context)
  2. Builds system prompt with package list + booking policy
  3. Calls Groq API (LLaMA 3.3 70B)
  4. Returns AI reply
         ↓
ChatWidget.jsx displays response with typing animation
```

**RAG-lite pattern:** The AI receives the actual package database as context in every request — so it can answer specific questions like *"Show me beach packages under ₹25,000"* or *"What is included in the Ladakh Trek?"* with real data, not hallucinated answers.
...
| `POST` | `/api/chat/message` | Public | AI travel chatbot |
